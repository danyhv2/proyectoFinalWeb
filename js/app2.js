	...
	var grupos = angular.module('grupos', ['ngMaterial', 'ngMessages', 'ui.bootstrap.demo','ui.router']);
		grupos.controller('gruposController', function($scope, $http){
			...
			$scope.addEstudiante=function(){
				$scope.muchos.push({nombre:$scope.nuevoEstudiante.name});
				$scope.temporales.push({nombre:$scope.nuevoEstudiante.name});
			...
			$scope.addGrupo=function(){
				$scope.grupos.push({name :$scope.nuevoName, estudiantes :$scope.muchos});
				$scope.nuevoName = '';
				$scope.nuevoEstudiante ='';
				$scope.muchos = [];
				//$scope.formVisibility=false;
